...
imp_birth <- did_imputation(
  ...
  horizon     = c(-10,10),                        # event-study: 5 years before & after treatment
...
ggplot(imp_birth_plot %>% filter(term >= -10 & term <= 10), 
...
ggplot(imp_birth_placebo_plot %>% filter(term >= -10 & term <= 6), 
       aes(x = as.numeric(term), y = estimate)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray") +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0.3, color = "blue") +
  geom_point(color = "blue", size = 2) +
