...
imp_birth <- did_imputation(
  ...
  horizon     = c(-10,10),                        # event-study: 10 years before & after treatment
...
imp_birth_plot_filled <- imp_birth_plot %>%
  filter(term >= -10 & term <= 10) %>%
  complete(term = seq(-10, 10)) %>%        # ensures all terms are present
  mutate(
...
ggplot(imp_birth_plot_filled %>% filter(term >= -10 & term <= 10), 
...
ggplot(imp_birth_placebo_plot_filled %>% filter(term >= -10 & term <= 6), 
       aes(x = as.numeric(term), y = estimate)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray") +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0.3, color = "blue") +
  geom_point(color = "blue", size = 2) +
...
ggplot(anticipation_results_birth %>% filter(term >= -10 & term <= 10), aes(x = term, y = estimate, color = anticipation_k, group = anticipation_k)) +
...
imp_employment <- did_imputation(
  ...
  tname       = "YEAR",                      # calendar year
  idname      = "STATEFIP",                  # cluster/unit ID
  horizon     = TRUE,                        # event-study: 10 years before & after treatment
  pretrends   = TRUE,                       # pre-treatment trends check
  cluster_var = "STATEFIP"                   # clustering
)
...
ggplot(imp_employed_plot_filled %>% filter(term >= -10 & term <= 10), 
       aes(x = as.numeric(term), y = estimate)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray") +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0.3, color = "blue") +
  geom_point(color = "blue", size = 2) +
...
ggplot(imp_employed_placebo_plot_filled %>% filter(term >= -10 & term <= 6), 
       aes(x = as.numeric(term), y = estimate)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray") +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0.3, color = "blue") +
  geom_point(color = "blue", size = 2) +
...
ggplot(anticipation_results_employment %>% filter(term >= -10 & term <= 10), aes(x = term, y = estimate, color = anticipation_k, group = anticipation_k)) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_ribbon(aes(ymin = lower, ymax = upper, fill = anticipation_k), alpha = 0.15, color = NA) +
  geom_line(size = 1) +
  geom_point(size = 1.5) +
